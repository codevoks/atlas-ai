...
| D34 | Phase 9 implements a deterministic local bounded research graph behind a `ResearchGraph` boundary before adopting a runtime dependency | Fixed implementation; LangGraph adapter evidence gate | The Phase 9 contract proves durable research state, checkpoints, idempotent tool records, budget limits, approval gates, tenant-scoped retrieval, and cited report synthesis without network-installed packages, paid APIs, or external tools. LangGraph remains the intended orchestration adapter boundary, but the repository does not require the package for the zero-cost product gate until dependency, runtime, checkpoint, and failure-mode evidence justify swapping the deterministic runner. Multi-agent research remains deferred until benchmark evidence shows material benefit over this bounded workflow. |
| D35 | Phase 10 uses deterministic local guardrails plus persisted security events as the default assurance baseline | Fixed implementation; advanced security evidence gate | Authentication, authorization, tenant scope, citation validation, research approval, input/output guardrails, egress policy, redaction, and quota enforcement must be executable in the zero-cost product gate. Pattern-based scanners and PostgreSQL fixed-window counters are deliberately simple and deterministic so failures are reproducible and auditable. Malware scanning, enterprise DLP/KMS/HSM, external red-team findings, Redis/distributed limits, hosted classifiers, and compliance controls remain evidence-gated production hardening work rather than unverified claims. |
| D36 | Phase 11 uses local no-content telemetry, protected operations posture, and plan-only infrastructure artifacts as the production-hardening baseline | Fixed final baseline; hosted operations evidence gate | Production readiness must be demonstrable without billable resources. The default stack records bounded route-level metrics and trace IDs locally, exposes admin/internal operational posture without raw content, validates CI/container/IaC artifacts, and documents SLOs/runbooks/capacity bottlenecks. Hosted observability, AWS provisioning, Terraform resources, OpenSearch, managed queues, domains, and deployment promotion remain opt-in work that requires explicit approval plus measured evidence; the final phase proves a defensible baseline rather than pretending every enterprise operation is already run in production. |
