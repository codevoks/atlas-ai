...
- `answer_evidence`: immutable evidence snapshot per answer run, including chunk/document/version/source identities, retrieval stage and scores, context rank, quote text, source span, and retrieval-plan/query-variant provenance.
...
- `evaluation_cases`: labeled offline test cases with query, search/answer mode, retrieval configuration version, top-k, relevant chunk labels, expected answer fragments, expected citation quotes, slice metadata, and optional reviewer labels.
